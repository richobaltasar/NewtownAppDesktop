...
namespace Ewats_App.Page
{
    public partial class MasterBank : Form
    {
        ...
        public void atur_grid()
        {
            ...
            dt_grid.Columns[0].Name = "Id";
            dt_grid.Columns[1].Name = "NamaEwallet";
            dt_grid.Columns[2].Name = "KodeWallet";
            dt_grid.Columns[3].Name = "AdminCharges";
            dt_grid.Columns[4].Name = "Diskon";
        ...
        {
            atur_grid();
            var data = f.GetDataWalet(search);
            int a = 0;
            foreach (var r in data)
            {
                a++;
                string[] row = new string[] { r.Id.ToString(), r.NamaEwallet, r.KodeWallet, f.ConvertToRupiah(Convert.ToDecimal(r.AdminCharges)), r.Diskon };
        ...
        private void dt_grid_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                var data = new Model.DataEwallet();
                DataGridViewRow row = this.dt_grid.Rows[e.RowIndex];

                data.Id = Convert.ToInt32(row.Cells["Id"].Value.ToString());
                data.KodeWallet = row.Cells["KodeWallet"].Value.ToString();
                data.NamaEwallet = row.Cells["NamaEwallet"].Value.ToString();
                data.Diskon = row.Cells["Diskon"].Value.ToString();
                data.AdminCharges = row.Cells["AdminCharges"].Value.ToString();
                ...
                if (data != null)
                {
                    ...
                    if (frm != null)
                    {
                        if (LblPage.Text == "ewallet")
                        {
                            ...
                            TextBox txtNominal = frm.Controls.Find("txtNominal", true).FirstOrDefault() as TextBox;
                            TextBox txtTotalTrx = frm.Controls.Find("txtTotalTrx", true).FirstOrDefault() as TextBox;
                            ...
                            if (txtBankCode != null)
                            {
                                txtBankCode.Text = data.KodeWallet;
                            }
                            if (txtNamaBank != null)
                            {
                                txtNamaBank.Text = data.NamaEwallet;
                            }
                            if (txtDiskon != null)
                            {
                            ...
                            {
                                TxtNominalDiskon.Text = f.ConvertToRupiah(TotalBelanja * f.ConvertDecimal(data.Diskon) / 100);
                                DiskonBankNominal = TotalBelanja * f.ConvertDecimal(data.Diskon) / 100;
                            }
                            ...
                            if (txtNominal != null)
                            {
                                txtTotalTrx.Text = f.ConvertToRupiah((f.ConvertDecimal(txtTotalBelanja.Text) - DiskonBankNominal) + AdminCharges);
                                txtNominal.Text = txtTotalTrx.Text;
                                txtNominal.Focus();
                            }
                            this.Close();
                        }
